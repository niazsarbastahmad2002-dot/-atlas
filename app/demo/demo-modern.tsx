...
export function ModernDemoWorkspace() {
  ...
  return (
    <div className="app-shell demo-modern-shell">
      <header className="app-topbar">
        <div className="app-topbar-inner shell">
          ...
          <div className="demo-modern-top-actions">
            <span className="demo-modern-pill">Sample clinic</span>
            <Link className="button button-ghost button-small" href="/login">Use Atlas</Link>
          </div>
        </div>
      </header>

      <div className="demo-modern-flow" role="status">
        <div className="shell demo-modern-flow-inner">
          <span><strong>Clinic timing</strong> · running on time</span>
          <span>Safe demo · nothing is saved</span>
        </div>
      </div>
      ...
      <main className="workspace-page shell demo-modern-workspace">
        <header className="workspace-header">
          <div className="workspace-title-block">
            ...
            <h1>Hawler Sample Clinic</h1>
            <p>Dr. Sara · synthetic data only</p>
          </div>
          <div className="demo-modern-clock">Today · Erbil</div>
        </header>

        <nav className="schedule-date-shortcuts" aria-label="Quick schedule dates">
          <button type="button" disabled>Yesterday</button>
          <button type="button" className="is-selected" aria-current="date">Today</button>
          <button type="button" disabled>Tomorrow</button>
        </nav>
        ...
        <section className="stats workspace-stats" aria-label="Today’s sample appointment summary">
          ...
          <Stat label="No-show" value={counts.noShow} />
        ...
        <div className="workspace-grid">
          <section className="panel appointment-composer" id="new-appointment">
            ...
            >
              <label htmlFor="demo_patient">Patient name</label>
              <input id="demo_patient" name="patient" minLength={2} maxLength={120} placeholder="Sample patient" required />
              <label htmlFor="demo_phone">Iraqi mobile number</label>
              <input id="demo_phone" name="phone" type="tel" inputMode="tel" placeholder="0750 000 0000" required />
              <label>Doctor</label>
              <div className="composer-doctor-lock"><strong>Dr. Sara</strong><span aria-hidden="true">✓</span></div>
              <label htmlFor="demo_time">Time <span className="label-muted">· Erbil</span></label>
              <input id="demo_time" name="time" type="time" defaultValue="11:30" required />
              <button className="button" type="submit">Save sample appointment</button>
            </form>
            <div className="reminder-note"><strong>Patient reminders: </strong>off in safe demo</div>
            <p className="composer-privacy">Scheduling only — do not enter real patient or medical information.</p>
          </section>

          <section className="panel appointments-panel schedule-card">
            <div className="panel-heading schedule-heading">
              <div><div className="eyebrow">Today</div><h2>Clinic day</h2><p className="panel-subtitle">Dr. Sara</p></div>
                  ...
                  <article className="appointment-row polished-appointment" key={appointment.id}>
                    <div className="appointment-primary">
                      <div className="patient-cell">
                        ...
                        <span><bdi dir="ltr">{appointment.phone}</bdi></span>
                      </div>
                      <div className="appointment-badges">
                        {order ? <span className="appointment-order-badge">#{order}</span> : null}
                    ...
                    <dl className="appointment-details polished-details">
                      <div><dt>Time</dt><dd className="appointment-time-value">{timeLabel(appointment.time)}</dd></div>
                      <div><dt>Doctor</dt><dd>{appointment.doctor}</dd></div>
                      <div><dt>Reminder</dt><dd>Safe demo · off</dd></div>
                    </dl>
                    <div className="row-actions polished-actions" aria-label={`Sample actions for ${appointment.patient}`}>
                      ...
                      {(appointment.status === "pending" || appointment.status === "confirmed") ? <button type="button" onClick={() => setStatus(appointment.id, "completed")}>Complete</button> : null}
                      {(appointment.status === "pending" || appointment.status === "confirmed") ? <button type="button" onClick={() => setStatus(appointment.id, "no_show")}>No-show</button> : null}
                      {(appointment.status === "pending" || appointment.status === "confirmed") ? <button type="button" onClick={() => setStatus(appointment.id, "cancelled")}>Cancel</button> : null}
                      {(appointment.status === "completed" || appointment.status === "no_show" || appointment.status === "cancelled") ? <button type="button" onClick={() => setStatus(appointment.id, "pending")}>Reopen</button> : null}
                    </div>
                  </article>
                );
              })}
