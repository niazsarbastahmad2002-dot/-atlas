...
const exactCopy: Record<Exclude<UiLocale, "en">, Record<string, string>> = {
  ...
  bd: {
    "Lead time": "دەمێ بەری وادەیێ",
    "Messaging approved": "پەیام هاتنە پەسەندکرن",
    "Provider approval pending": "چاڤەڕێیا پەسەندکرنا دابینکەری",
    "Owner access": "تەنێ خودانێ کلینیکێ",
    "The clinic settings could not load.": "ڕێکخستنێن کلینیکێ بار نەبوون.",
    "Only the clinic owner or a manager can change clinic settings.": "تەنێ خودانێ کلینیکێ یان بەڕێڤەبەر دشێت ڕێکخستنان بگۆڕیت.",
    "Choose a supported interface language.": "زمانەکێ بەردەست هەلبژێرە.",
    "Check the clinic name and try again.": "ناڤێ کلینیکێ بپشکنە و دووبارە هەول بدە.",
    "Choose a valid appointment interval.": "ماوەیەکا دروست بۆ ناڤبەرا وادەیان هەلبژێرە.",
    "Check the doctor details and try again.": "زانیاریێن دکتۆری بپشکنە و دووبارە هەول بدە.",
    "Check the reminder settings and try again.": "ڕێکخستنێن بیرخستنەوەیێ بپشکنە و دووبارە هەول بدە.",
    "That setting could not be saved. Refresh and try again.": "ڕێکخستن نەهاتە پاراستن. دووبارە هەول بدە.",
    "Interface language updated.": "زمانێ Atlas هاتە گۆڕین.",
    "Clinic details updated.": "زانیاریێن کلینیکێ هاتنە نوێکرن.",
    "Appointment interval updated.": "ماوەیا ناڤبەرا وادەیان هاتە نوێکرن.",
    "Doctor settings updated.": "ڕێکخستنێن دکتۆری هاتنە نوێکرن.",
    "Doctor archived. Existing appointment history is preserved.": "دکتۆر هاتە ئەرشیڤکرن؛ مێژوویا وادەیان پاراستییە.",
    "Doctor restored.": "دکتۆر هاتە ڤەگەراندن.",
    "Reminder settings updated.": "ڕێکخستنێن بیرخستنەوەیێ هاتنە نوێکرن.",
  },
  ar: {
...
function localizeVisibleText(value: string, locale: UiLocale) {
  ...
  let text = value;
  if (locale === "ku") {
    text = text.replace(/\b(\d+)\s*min\b/gi, (_, n: string) => `${n} خولەک`).replace(/\b(\d+)\s*hours?\b/gi, (_, n: string) => `${n} کاتژمێر`).replace(/\b(\d+)\s*days?\b/gi, (_, n: string) => `${n} ڕۆژ`).replace(/\bTemplate:\s*/g, "قاڵب: ").replace(/\blimit\s+/gi, "سنوور ").replace(/\/day\b/gi, "/ڕۆژ");
  } else if (locale === "bd") {
    text = text.replace(/\b(\d+)\s*min\b/gi, (_, n: string) => `${n} خولەک`).replace(/\b(\d+)\s*hours?\b/gi, (_, n: string) => `${n} دەمژمێر`).replace(/\b(\d+)\s*days?\b/gi, (_, n: string) => `${n} ڕۆژ`).replace(/\bTemplate:\s*/g, "قاڵب: ").replace(/\blimit\s+/gi, "سنوور ").replace(/\/day\b/gi, "/ڕۆژ");
  } else {
    text = text.replace(/\b(\d+)\s*min\b/gi, (_, n: string) => `${n} دقيقة`).replace(/\b(\d+)\s*hours?\b/gi, (_, n: string) => `${n} ساعة`).replace(/\b(\d+)\s*days?\b/gi, (_, n: string) => `${n} يوم`).replace(/\bTemplate:\s*/g, "القالب: ").replace(/\blimit\s+/gi, "الحد ").replace(/\/day\b/gi, "/يوم");
  }
...
export function DashboardClientPolish({ locale }: { locale: UiLocale }) {
  ...
  return (
    <style jsx global>{`
      .appointment-composer { position: static !important; top: auto !important; }
      .workspace-page { padding-bottom: calc(120px + env(safe-area-inset-bottom, 0px)); }
      .appointment-composer, .appointment-form, .app-content { overflow: visible; }
      .atlas-fast-save-toast { position: fixed; z-index: 250; inset-inline-end: max(18px, env(safe-area-inset-right, 0px)); bottom: max(22px, calc(env(safe-area-inset-bottom, 0px) + 18px)); display: grid; gap: 3px; width: min(320px, calc(100vw - 36px)); border: 1px solid #cbdad1; border-radius: 14px; padding: 12px 14px; background: #fff; color: var(--ink); box-shadow: 0 18px 48px rgba(20,36,28,.16); animation: atlas-toast-in .16s ease-out both; }
      ...
      @keyframes atlas-toast-in { from { opacity: 0; transform: translateY(8px) scale(.985); } to { opacity: 1; transform: translateY(0) scale(1); } }
      @media (max-width: 720px) { .atlas-fast-save-toast { inset-inline: 14px; width: auto; bottom: calc(84px + env(safe-area-inset-bottom, 0px)); } }
    `}</style>
  );
}
