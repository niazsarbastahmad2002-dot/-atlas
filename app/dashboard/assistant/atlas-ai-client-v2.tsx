...
const copy: Record<UiLocale, Copy> = {
  en: {
    ...
    title: "Atlas AI",
    subtitle: "Simple help for clinic work, writing, planning, translation, and everyday questions.",
    beta: "Beta · read-only",
    ...
    liveHint: "Talk naturally. Atlas listens, answers, and listens again.",
    listening: "Listening",
    ...
    tryAgain: "Try again",
    endVoice: "End",
    voiceUnavailable: "Voice is not available right now. Try again.",
    voicePermission: "Atlas needs microphone permission for voice.",
    noSpeech: "I didn't hear enough speech. Try again.",
    unclearSpeech: "I heard you, but not clearly enough to write it reliably. Try again a little closer to the microphone.",
    micOn: "Microphone is hearing you",
    micQuiet: "Microphone is on — speak now",
    voiceOutputApproximate: "This device has no native Kurdish voice, so Atlas is using the closest available voice.",
    replay: "Listen",
    you: "You",
    presets: [
  ...
  ku: {
    ...
    title: "Atlas AI",
    subtitle: "یارمەتیی سادە بۆ کاری کلینیک، نووسین، پلان، وەرگێڕان و پرسیاری ڕۆژانە.",
    ...
    endVoice: "کۆتایی",
    voiceUnavailable: "دەنگ ئێستا بەردەست نییە. دووبارە هەوڵ بدە.",
    voicePermission: "Atlas پێویستی بە ڕێگەی مایکرۆفۆن هەیە.",
    noSpeech: "قسەی پێویستم نەبیست. دووبارە هەوڵ بدە.",
    unclearSpeech: "گوێم لێت بوو، بەڵام بە ڕوونی نەبوو بۆ نووسینی دڵنیابوو. کەمێک نزیکتر بە مایکرۆفۆن دووبارە قسە بکە.",
    micOn: "مایکرۆفۆن دەنگت دەگرێت",
    micQuiet: "مایکرۆفۆن کراوەیە — قسە بکە",
  ...
  bd: {
    ...
    title: "Atlas AI",
    subtitle: "هاریکاریا سادە بۆ کارێ کلینیکێ، نڤیسین، پلان، وەرگێڕان و پسیارێن ڕۆژانە.",
    ...
    busy: "Atlas AI نها مژوولە. پشتی کەمەکێ دیسان هەول بدە.",
    newChat: "گفتوگۆیا نوی",
    dictate: "دەنگ بۆ نڤیسینێ",
    stopDictation: "بوەستینە",
    ...
    transcribing: "ئاخفتنا تە تێدگەهیت…",
    speaking: "Atlas دئاخڤیت",
    paused: "دەنگ راوەستیا",
    sendNow: "نها بفرێنە",
    interrupt: "بوەستینە",
    tryAgain: "دیسان هەول بدە",
    endVoice: "دوماهیک",
    voiceUnavailable: "دەنگ نها بەردەست نینە. دیسان هەول بدە.",
    voicePermission: "Atlas پێدڤی ب ڕێکا مایکرۆفۆنێ هەیە.",
    noSpeech: "من ئاخفتنا پێدڤی نەگرت. دیسان هەول بدە.",
    unclearSpeech: "من گوهداریا تە کر، لێ بۆ نڤیسینا دڵنیابوو زۆر ڕوون نەبوو. کەمەکێ نزیکتر ب مایکرۆفۆنێ دیسان باخڤە.",
    micOn: "مایکرۆفۆن دەنگێ تە دگریت",
    micQuiet: "مایکرۆفۆن ڤەکرییە — باخڤە",
  ...
  ar: {
    eyebrow: "مساعد Atlas",
    title: "Atlas AI",
    subtitle: "مساعدة بسيطة لشغل العيادة، الكتابة، التخطيط، الترجمة والأسئلة اليومية.",
    ...
    busy: "Atlas AI مشغول حالياً. حاول بعد شوي.",
    newChat: "محادثة جديدة",
    dictate: "إملاء صوتي",
    stopDictation: "إيقاف",
    dictateHint: "احچي، راجع الكلام المكتوب، وبعدها إنت اضغط إرسال.",
    ...
    endVoice: "إنهاء",
    voiceUnavailable: "الصوت غير متاح حالياً. حاول مرة ثانية.",
    voicePermission: "Atlas يحتاج إذن المايكروفون حتى يستخدم الصوت.",
    noSpeech: "ما سمعت كلام كافي. حاول مرة ثانية.",
    unclearSpeech: "سمعتك، بس الكلام مو واضح كفاية حتى أكتبه بثقة. قرب شوي من المايكروفون وحاول مرة ثانية.",
    micOn: "المايكروفون يسمعك",
    micQuiet: "المايكروفون شغال — احچي",
  ...
  return (
    <div className="atlas-ai-rich-text">
      {text.replace(/\r/g, "").split("\n").map((raw, index) => {
        ...
        if (/^[-*]\s+/.test(line)) {
          return <div className="atlas-ai-bullet" key={`b-${index}`}><span>•</span><p>{inlineRichText(line.replace(/^[-*]\s+/, ""))}</p></div>;
        }
...
export function AtlasAiClient({ clinicId, clinicName, locale }: AtlasAiClientProps) {
  ...
  const captureRef = useRef<AtlasPcmCapture | null>(null);
  const capturePurposeRef = useRef<RecordingPurpose | null>(null);
  ...
  useEffect(() => () => {
    ...
    if (capture) void capture.discard();
  ...
  async function microphoneStream(purpose: RecordingPurpose) {
    ...
    if (purpose === "live" && streamRef.current?.active) return streamRef.current;
    try {
      ...
      if (purpose === "live") streamRef.current = stream;
      return stream;
    } catch (caught) {
  ...
  function onMicLevel(level: number, purpose: RecordingPurpose) {
    ...
    if (level > 0.055) {
      ...
      setMicQuiet(false);
    } else if (now - lastAudibleAtRef.current > 3200) {
      setMicQuiet(true);
    }
    ...
    if (
      ...
      && captureRef.current
      && now - lastSpeechAtRef.current > 1750
      && now - captureRef.current.startedAt > 900
    ) {
  ...
  async function startCapture(purpose: RecordingPurpose) {
    ...
    lastAudibleAtRef.current = performance.now();
    if (purpose === "live") setLiveDisplayText("");

    let stream: MediaStream;
    try {
      stream = await microphoneStream(purpose);
    } catch (caught) {
    ...
    try {
      const capture = await startAtlasPcmCapture(stream, (level) => onMicLevel(level, purpose));
      captureRef.current = capture;
      capturePurposeRef.current = purpose;
      setVoiceStage("listening");
      if (purpose === "dictation") setDictating(true);
      clearMaxTimer();
      maxTimerRef.current = window.setTimeout(() => void stopCapture(false), purpose === "live" ? 30_000 : 45_000);
    } catch {
  ...
  async function stopCapture(discard: boolean) {
    ...
    if (!capture || !purpose) return;

    stoppingRef.current = true;
    captureRef.current = null;
    capturePurposeRef.current = null;
    clearMaxTimer();
    setMicLevel(0);
    ...
    try {
      if (discard) {
        ...
        if (purpose === "dictation") stopTracks(streamRef.current);
        setVoiceStage("idle");
        return;
      }
      ...
      if (purpose === "dictation") {
        // Dictation uses a one-turn stream, unlike Live Voice which keeps the same mic permission/session.
        stopTracks(streamRef.current);
      }
      if (blob.size < 1000 || !speechSeenRef.current) {
  ...
  async function transcribeAudio(blob: Blob) {
    ...
    form.append("audio", new File([blob], "atlas-voice.wav", { type: "audio/wav" }));
    ...
    if (!response.ok || !payload.text?.trim()) {
      if (payload.error === "no_speech") throw new Error("no_speech");
      if (payload.error === "unclear_speech") throw new Error("unclear_speech");
  ...
  async function finishRecordedTurn(blob: Blob, purpose: RecordingPurpose) {
    ...
    try {
      ...
      const code = caught instanceof Error ? caught.message : "voice_unavailable";
      setVoiceError(code === "no_speech" ? t.noSpeech : code === "unclear_speech" ? t.unclearSpeech : code === "busy" ? t.busy : t.voiceUnavailable);
      setVoiceStage("idle");
      // Deliberately do NOT auto-restart after a failed transcription. The user sees the error and explicitly retries.
    }
  }
  ...
  async function speakAnswer(text: string, continueConversation: boolean) {
    ...
    const selected = chooseVoice(desiredSpeechLanguage(locale));
    if (selected.voice) {
      utterance.voice = selected.voice;
      utterance.lang = selected.voice.lang;
    } else {
      utterance.lang = desiredSpeechLanguage(locale);
    }
    utterance.rate = 0.98;
    utterance.pitch = 1;
    utterance.volume = 1;
    ...
    const finish = () => {
      ...
      if (continueConversation && liveVoiceRef.current) {
        window.setTimeout(() => {
          if (liveVoiceRef.current && !voiceError) void startCapture("live");
        }, 500);
      }
    };
  ...
  async function askAtlas(value: string, fromLiveVoice = false) {
    ...
    if (nextQuestion.length < 2 || loadingRef.current) return null;
    ...
    try {
      const response = await fetch("/api/atlas-ai", {
        ...
        body: JSON.stringify({
          clinicId,
          locale,
          interaction: fromLiveVoice ? "voice" : "text",
      ...
      const assistantMessage: ChatMessage = { id: makeId(), role: "assistant", content: payload.answer, mode: payload.mode };
      ...
      const message = code === "busy" ? t.busy : t.error;
      setError(message);
      if (fromLiveVoice) setVoiceError(message);
      setVoiceStage("idle");
      return null;
    } finally {
      setLoadingValue(false);
  ...
  function toggleDictation() {
    ...
    if (liveVoiceRef.current || loadingRef.current || stoppingRef.current) return;
    setError(null);
    setVoiceError(null);
    void startCapture("dictation");
  }

  function startLiveVoice() {
    if (loadingRef.current || dictating || liveVoiceRef.current || stoppingRef.current) return;
  ...
  function retryLiveVoice() {
    if (!liveVoiceRef.current || loadingRef.current || captureRef.current || speakingRef.current || stoppingRef.current) return;
    setVoiceError(null);
    void startCapture("live");
  }
  ...
  function newChat() {
    endLiveVoice();
    if (captureRef.current) void stopCapture(true);
    setDictating(false);
    setMessages([]);
    messagesRef.current = [];
    setQuestion("");
  ...
  const micStatus = micQuiet ? t.micQuiet : t.micOn;

  return (
    <section className="atlas-ai-card" aria-labelledby="atlas-ai-title">
      <div className="atlas-ai-heading">
        <div><div className="eyebrow">{t.eyebrow}</div><h1 id="atlas-ai-title">{t.title}</h1><p>{t.subtitle}</p></div>
        <div className="atlas-ai-heading-actions"><span className="atlas-ai-beta">{t.beta}</span><button type="button" onClick={newChat}>{t.newChat}</button></div>
      </div>
      ...
      <div className="atlas-ai-thread" aria-live="polite">
        {messages.length === 0 ? (
          <div className="atlas-ai-empty">
            <div className="atlas-ai-orb">A</div><strong>{t.title}</strong><span>{t.examples}</span>
            <div className="atlas-ai-presets">{t.presets.map((preset) => <button key={preset} type="button" onClick={() => void askAtlas(preset)} disabled={loading}>{preset}</button>)}</div>
          </div>
          ...
          <article key={message.id} className={`atlas-ai-message is-${message.role}`}>
            ...
            {message.role === "assistant" ? <button className="atlas-ai-listen-button" type="button" onClick={() => void speakAnswer(message.content, false)}><SpeakerIcon /> {t.replay}</button> : null}
          </article>
        ))}
        {loading ? <article className="atlas-ai-message is-assistant is-thinking"><div className="atlas-ai-message-role">Atlas AI</div><p><i /><i /><i /> {t.thinking}</p></article> : null}
      ...
      <div className="atlas-ai-voice-tools">
        <button className="atlas-ai-live-button" type="button" onClick={startLiveVoice} disabled={loading || dictating}><span className="atlas-ai-live-dot" />{t.liveVoice}</button>
        <button className={`atlas-ai-dictate-button ${dictating ? "is-on" : ""}`} type="button" onClick={toggleDictation} disabled={loading || liveVoice}>{dictating ? <StopIcon /> : <MicIcon />}{dictating ? t.stopDictation : t.dictate}</button>
        <div className="atlas-ai-mic-mini" aria-hidden="true"><span style={{ transform: `scaleY(${Math.max(.12, micLevel)})` }} /><span style={{ transform: `scaleY(${Math.max(.2, micLevel * .8)})` }} /><span style={{ transform: `scaleY(${Math.max(.1, micLevel * .65)})` }} /></div>
        <span className="atlas-ai-dictate-hint">{dictating ? micStatus : t.dictateHint}</span>
      </div>

      <form className="atlas-ai-composer" onSubmit={submit}>
        <textarea value={question} onChange={(event) => setQuestion(event.target.value)} onKeyDown={composerKeyDown} placeholder={t.placeholder} maxLength={1200} rows={2} aria-label={t.placeholder} disabled={loading || voiceStage === "transcribing"} />
        <div className="atlas-ai-composer-actions">
          <button className={`atlas-ai-mic-button ${dictating ? "is-on" : ""}`} type="button" onClick={toggleDictation} aria-label={dictating ? t.stopDictation : t.dictate} disabled={loading || liveVoice}>{dictating ? <StopIcon /> : <MicIcon />}</button>
          <button className="atlas-ai-send-button" type="submit" disabled={loading || dictating || voiceStage === "transcribing" || question.trim().length < 2} aria-label={t.send}><SendIcon /></button>
        </div>
      </form>

      {liveVoice ? (
        <div className="atlas-ai-live-overlay" role="dialog" aria-modal="true" aria-label={t.liveTitle}>
          <div className="atlas-ai-live-topbar"><div><span>ATLAS</span><strong>{t.liveTitle}</strong></div><button type="button" onClick={endLiveVoice}>{t.endVoice}</button></div>
          <div className={`atlas-ai-live-center is-${voiceStage}`}>
            <div className="atlas-ai-live-orb" style={{ "--atlas-mic-level": Math.max(.08, micLevel) } as React.CSSProperties}><span>A</span><i /><i /><i /></div>
            <strong className="atlas-ai-live-state">{voiceStatus}</strong>
            {voiceStage === "listening" ? <div className={`atlas-ai-live-mic-state ${micQuiet ? "is-quiet" : ""}`}>{micStatus}</div> : null}
            {liveDisplayText ? <div className="atlas-ai-live-transcript">{stripForSpeech(liveDisplayText).slice(0, 500)}</div> : <div className="atlas-ai-live-transcript is-hint">{t.liveHint}</div>}
          ...
          <div className="atlas-ai-live-controls">
            {voiceStage === "listening" ? <button className="is-primary" type="button" onClick={() => void stopCapture(false)}>{t.sendNow}</button> : null}
            {voiceStage === "speaking" ? <button className="is-primary" type="button" onClick={interruptAtlas}>{t.interrupt}</button> : null}
            {voiceStage === "idle" ? <button className="is-primary" type="button" onClick={retryLiveVoice}>{t.tryAgain}</button> : null}
