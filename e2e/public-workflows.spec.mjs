...
test("safe demo creates and changes a synthetic appointment without contacting Supabase", async ({ page }) => {
  ...
  await expect(page.getByText("Safe demo · nothing is saved")).toBeVisible();
  ...
  const appointment = page.locator("article.appointment-row").filter({ hasText: "Dilan Karim" });
  await expect(appointment).toContainText("0750 123 4567");
