...
begin
  ...
  select net.http_post(
    ...
    body := jsonb_build_object('activate', p_activate),
    headers := jsonb_build_object(
      'Authorization', 'Bearer ' || v_raw_token,
      'Content-Type', 'application/json',
      'User-Agent', 'atlas-supabase-readiness/2.0'
    ),
...
comment on function private.invoke_atlas_meta_readiness(boolean) is
  'Creates a short-lived one-time token and invokes the protected Atlas Meta readiness endpoint without exposing provider or scheduler credentials.';
