...
test("settings launches Meta Coexistence and completes it server-side without activating reminders", () => {
  const panel = readFileSync("app/dashboard/whatsapp-coexistence-panel.tsx", "utf8");
  const card = readFileSync("app/dashboard/settings-reminder-card.tsx", "utf8");
  const launch = readFileSync("lib/reminders/meta-coexistence.ts", "utf8");

  assert.match(card, /WhatsAppCoexistencePanel/);
