...
test("App Store icon is a complete opaque 1024px PNG", async () => {
  ...
  while (offset < png.length) {
    ...
    assert.ok(crcEnd <= png.length);
    const type = png.subarray(typeStart, dataStart).toString("ascii");
    assert.equal(crc32(png.subarray(typeStart, dataEnd)), png.readUInt32BE(dataEnd));
    if (type === "IHDR") {
    ...
    if (type === "IEND") {
      reachedEnd = true;
      assert.equal(crcEnd, png.length);
      break;
    }
    offset = crcEnd;
  }
...
test("account deletion is in-app, retry-safe, and atomically cascades owned clinics", async () => {
  const [accountAction, accountPage, cascadeMigration, phoneManager] = await Promise.all([
    read("app/dashboard/settings/account/actions.ts"),
    read("app/dashboard/settings/account/page.tsx"),
    read("supabase/migrations/20260824050500_whatsapp_auth_atomic_identity_account_cascade.sql"),
    read("app/dashboard/settings/phone-number-manager.tsx"),
  ]);
  ...
  assert.doesNotMatch(accountAction, /from\("clinics"\)[\s\S]*\.delete\(\)/);
  assert.ok(accountAction.indexOf("auth.admin.deleteUser") < accountAction.indexOf("revokeAppleAuthorization(appleCredential)"));
  ...
  assert.match(cascadeMigration, /references auth\.users\(id\) on delete cascade/i);
  assert.match(phoneManager, /Account & deletion/);
  assert.match(phoneManager, /\/dashboard\/settings\/account/);
});
